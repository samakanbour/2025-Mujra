...
def draw_solution(G: nx.DiGraph, flows: Dict[Edge, float], node_cap: Dict[Node, float], pipe_cap: Dict[Edge, float]):
    """Draw the network solution with professional blues styling and informative color coding"""
    plt.figure(figsize=(14, 10), facecolor='white')  # Larger figure size
    ...
    scale = 1.2  
    for node in pos:
        pos[node] = [coord * scale for coord in pos[node]]
    
    # Add a subtle background
    plt.gca().set_facecolor('#f8f9fa')
    
    # Color maps -----------------------------------------------------------
    blues_cmap = plt.cm.Blues  # Use Blues for both nodes and edges
    edge_vmin, edge_vmax = 0.2, 1.0  # Restrict color range for edges for better contrast

    # Draw edges first -----------------------------------------------------
    widths = [3.0 + 6 * edge_util[e] for e in G.edges]  # Thicker edges
    edges = nx.draw_networkx_edges(
        G, pos,
        width=widths,
        edge_color=[edge_util[e] for e in G.edges],
        edge_cmap=blues_cmap,
        edge_vmin=edge_vmin, 
        edge_vmax=edge_vmax,
        arrows=True, 
        arrowstyle='-|>',
    ...
    nodes = nx.draw_networkx_nodes(
        G, pos,
        node_color=[node_util[n] for n in G.nodes],
        node_size=1800,
        node_shape='o',
        cmap=blues_cmap,
        edgecolors='white',
    ...
    # Add a colorbar for node utilization -----------------------------------
    sm = plt.cm.ScalarMappable(cmap=blues_cmap)
    sm.set_array([])
    cbar = plt.colorbar(sm, ax=plt.gca(), shrink=0.8, pad=0.03)
    cbar.set_label('Node Utilization', fontsize=14, fontweight='bold')
    ...
    # Legend and titles -----------------------------------------------------
    plt.axis("off")
    plt.title("Network Flow Optimization Solution", fontsize=20, fontweight='bold', pad=20)
    
    plt.figtext(0.5, 0.02, "Edge labels: flow / capacity", 
    ...
    blue_patch = plt.Line2D([0], [0], marker='o', color='w', 
                            markerfacecolor=blues_cmap(0.7), markersize=15, label='Node')
    edge_patch = plt.Line2D([0], [0], color=blues_cmap(0.7), linewidth=4, label='Edge')
    plt.legend(handles=[blue_patch, edge_patch], loc='upper right', fontsize=14)
